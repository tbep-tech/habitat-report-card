...
```{r}
prodat <- filter(rstdatall, Activity == 'Protection')
protot <- nrow(prodat)
procur <- prodat %>% filter(Year == cur) %>% nrow()
```

##### **`r protot` protection projects** to date.

##### **`r procur` protection projects** in `r cur`.
...
### Total `r cur` as stacked barplot

```{r, out.width = '50%'}
knitr::include_graphics(here('docs/figs/totalbar.png'))
```
...
```{=html}
<iframe height="1050" width="100%" src="https://tbep-tech.github.io/hmpu-workflow/target_table_simple.html"></iframe>
```
