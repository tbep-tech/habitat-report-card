---
title: "Oyster Bay Segment Targets"
...
```{r}
#| fig-cap: "Oyster Acres by Bay Segment with OHSI Targets"
#| fig-width: 7
#| fig-height: 7
...
ggplot(toploop1, aes(x = yr, y = acres)) + 
  ...
  scale_linetype_manual(name = 'OHSI Targets', values = c('15 only' = 'dashed', '14 and 15' = 'dotted')) +
  theme_minimal() +
  theme(
    legend.position = 'top',
...
reactable(totab, 
          columns = list(
            ...
            thrsh = colDef(name = 'Target', footer = function(values) htmltools::tags$b(format(round(sum(values), 0), big.mark = ','))),
            Total = colDef(footer = function(values) htmltools::tags$b(format(round(sum(values), 0), big.mark = ',')))
          ),
          defaultColDef = colDef(
            format = colFormat(separators = TRUE, digits = 0), 
...
```{r}
#| fig-cap: "Oyster Acres by Bay Segment with Target Proportional to Maximum Observed"
#| fig-width: 7
#| fig-height: 7
...
ggplot(toploop2, aes(x = yr, y = acres)) + 
  ...
  facet_wrap(~segment, ncol = 3) +
  scale_x_continuous(breaks = c(2014, 2016, 2018, 2020, 2022, 2024)) +
  geom_hline(aes(yintercept = thrsh, linetype = 'Target'), color = 'tomato1', linewidth = 1) +
